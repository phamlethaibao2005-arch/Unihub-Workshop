...
export function Nav() {
  ...
  return (
    <header className="sticky top-0 z-50 flex h-14 items-center border-b border-hairline bg-canvas px-4 md:px-6 lg:px-10">
      <Link
        href="/"
        className="mr-8 shrink-0 font-display text-[22px] tracking-[0.04em] text-ink"
      >
        UNIHUB
      </Link>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        {session?.user ? (
          ...
          <Link href="/login" className="text-[13px] font-semibold text-ink underline underline-offset-4">
            Đăng nhập
          </Link>
        )}
      </div>
